	...
	public static void getAllUnapprovedBusinessAccount(int branchManagerID, Response response) {
		...
		try {
			getAllBusinessAccount = EchoServer.con.prepareStatement(
					"SELECT UserName FROM biteme.account AS account WHERE EXISTS"
					+ " (SELECT * FROM biteme.business_account AS business WHERE business.UserName = account.UserName AND account.BranchManagerID = ? AND business.isApproved = 0);");
