...
public class PriceVolume extends JPanel implements ChartMouseListener // A demo application for price-volume chart.
{
	...
	public PriceVolume(int branchmanagerID, HashMap<LocalDateTime, Integer> datesToOrders,
			HashMap<LocalDateTime, Double> datesToIncome, int quarter) {
		LocalDateTime now = LocalDateTime.now();
		DateTimeFormatter year = DateTimeFormatter.ofPattern("yyyy");;
		String Symbol = "Quarter: " + quarter + " report for branch number: " + branchmanagerID;
		JFreeChart chart = createChart(Symbol, datesToOrders, datesToIncome);
		...
		temp = new StringBuilder();
		temp.append("quarter_");
		temp.append(quarter);
		temp.append("_");
		temp.append(branchmanagerID);
		temp.append("_");
		temp.append(now.format(year));
		temp.append(".png");
		try {
			ChartUtils.saveChartAsPNG(new File(QueryConsts.FILE_PATH_REPORTS+temp.toString()), chart, 1000, 600);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		/*
		 * xCrosshair.setValue(1.5959952E12); xCrosshair.setVisible(true);
		 * yCrosshair.setValue(45.230579); yCrosshair.setVisible(true);
		 */
	}
	String head = new String();
	private JFreeChart createChart(String Symbol, HashMap<LocalDateTime, Integer> datesToOrders, HashMap<LocalDateTime, Double> datesToIncome) {
		createPriceDataset(Symbol, datesToOrders, datesToIncome);
		XYDataset priceData = new TimeSeriesCollection(Price_series);
		JFreeChart chart = ChartFactory.createTimeSeriesChart(head, "Date", getYLabel("Price($)"), priceData, true,
				true, true);
		...
		plot.getRenderer().setDefaultToolTipGenerator(
				new StandardXYToolTipGenerator(StandardXYToolTipGenerator.DEFAULT_TOOL_TIP_FORMAT,
						new SimpleDateFormat("yyyy-MM-d"), NumberFormat.getCurrencyInstance()));

		NumberAxis rangeAxis2 = new NumberAxis("Amount");
		...
		DateAxis domainAxis = (DateAxis) plot.getDomainAxis(); // Consider adjusting the lower margin of the domain axis
																// for symmetry.
	...
	private void createPriceDataset(String Symbol, HashMap<LocalDateTime, Integer> datesToOrders, HashMap<LocalDateTime, Double> datesToIncome) {
		String Lines[] = new String[21], Items[], Date;
		int Year, Month, Day;
		long Volume, Last_Volume = 0;
		double Price, Last_Price=0;
...
//		}
		...
		int totalSales=0;
		for(LocalDateTime date : datesToOrders.keySet()) {
			totalSales+=Volume = datesToOrders.get(date);
			Year = Integer.parseInt(date.format(yearf));
			Month = Integer.parseInt(date.format(monthf));
			Day = Integer.parseInt(date.format(dayf));
			Volume_Series.add(new Day(Day, Month, Year), Volume);
		}
		for(LocalDateTime date : datesToIncome.keySet()) {
			totalIncome+=Price = datesToIncome.get(date);
			Year = Integer.parseInt(date.format(yearf));
			Month = Integer.parseInt(date.format(monthf));
			Day = Integer.parseInt(date.format(dayf));
			Price_series.add(new Day(Day, Month, Year), Price);
			Volume_Color_Vector.add(Price >= Last_Price ? "+" : "-");
			Last_Price = Price;
		}
		head = Symbol + "\n" + "Total sales: " + totalSales + " Total Income: " + String.format("%.2f$", totalIncome);
	}
	...
	String getYLabel(String Text) {
		...
		for (int i = 0; i < Text.length(); i++)
			Result += Text.charAt(i) + (i < Text.length() - 1 ? "" : "");
//    Out(Result);
		return Result;
	}
...
//    System.out.println(row+" "+col+" "+super.getItemPaint(row,col));
		return PriceVolume.Volume_Color_Vector.elementAt(col).equals("+") ? super.getItemPaint(row, col)
